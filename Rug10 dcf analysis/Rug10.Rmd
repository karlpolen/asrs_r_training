---
title: "Time Series and Discounted Cash Flow Models"
output:
...
### Abstract
In this post we illustrate some techniques for building cash flow models using zoo objects.  We also demonstrate methods to analyze cash flows including net present value, IRR, payback and graphical methods.  

### Some preliminary comments on zoo objects and time indices

Zoo objects provide a way of representing time series information.  They require at most one entry for each time period.  If you are analyzing a problem with multiple entries for the same time period, you will need to use a data frame with separate columns for the date and value entries.  
...
A zoo object is a vector or matrix with a timeline as an attribute (attached to the rows if a matrix).  While the zoo framework is quite general in what it will accept as an index, we will confine our discussion to cases where you are considering annual, quarterly, monthly and daily data.  Zoo also works with POSIXct class if you want to record intraday with a granularity as small as microseconds.  Other packages allow greater granularity for high frequency data.
...
You can do date arithmetic with the date class with some help from lubridate.

```{r}
dates=as.Date("2016-1-1")+months(0:11) #a vector of the first day in each month in 2016
dates
wday(dates)  #what day of the week is the first day of each month
dates+(6-wday(dates))%%7 #use modulo arithmetic to determine the first friday of each month
wday(dates+(6-wday(dates))%%7)
wday(dates+(1-wday(dates))%%7) #change to get the first monday of each month
ldates=as.Date("2016-1-1")+months(1:12)-1 #the last day of each month in 2016
ldates-(wday(ldates)-6)%%7 #the last friday of each month in 2016
wday(ldates-(wday(ldates)-6)%%7)
```

With the help of Bloomberg, we can determine trading days.  The following provides a list of trading days for 2005 through 2025.  We then calculate the first and last trading days of each month in 2016.

```{r, message=FALSE, warning=FALSE}
#figure trading days (from 2005 to 2025)
require(Rbbg)
conn=blpConnect(verbose=FALSE)
ticker='SPX Index'
field='CALENDAR_NON_SETTLEMENT_DATES'
nontrade=bds(conn,ticker,field)
nontrade=as.Date(nontrade$`Holiday Date`,format='%Y-%m-%d')
nontrade=nontrade[nontrade<=as.Date("2025-1-1")]
nontrade=nontrade[nontrade>=as.Date("2005-1-1")]
alldays=as.Date("2005-1-1")+(0:(20*366))
alldays=alldays[!(alldays%in%nontrade)] #get rid of holidays
alldays=alldays[!(wday(alldays)==1)] #eliminate Sundays
tradingdays=alldays[!(wday(alldays)==7)] #eliminate Saturdays
trash=blpDisconnect(conn)
td2016=tradingdays[year(tradingdays)==2016] #trading days in 2016
aggregate(td2016,by=list(month(td2016)),min)$x #the first trading day of each month
aggregate(td2016,by=list(month(td2016)),max)$x #the last trading day of each month
```
...
You can use aggregate on zoo objects.  Say you have a payroll of $10,000 every other Friday and you want to budget the monthly amount for the next couple years.  Here is how you make this calculation.
...
```{r, warning=FALSE, message=FALSE}
cumcf=cumsum(cf.w$Total)
#created a function called "unzoo" to convert from zoo object to long data frame as needed by ggplot
#if curious, see code in the dcf_funs file
ggplot(unzoo(cumcf),aes(x=Date,y=Value))+
  geom_line()+
  xlab("")+
  ylab("Cumulative Cash Flow")
```
...
tvpi=100*cumval/investment
ggplot(unzoo(tvpi),aes(x=Date,y=Value))+
  geom_line()+
  xlab("")+
  ylab("TVPI")
```
...
irr.ans.z=zoo(100*irr.ans,daterange)
ggplot(unzoo(irr.ans.z),aes(x=Date,y=Value))+
  geom_line()+
  ylab("Percent")+
  xlab("")+
  ggtitle("IRR as a function of hold period")
```

So, the peak IRR is achieved if you had sold the house to an unsuspecting person right before the air conditioner went out!
...
Play with the rental house discounted cash flow model.  Add additional expenses.  Say you don't want to deal with the tenant and hire a property management that charges 8% of rents to handle the property.  What's that do to your IRR?  Try adding a different valuation metric, such as gross rent muliplier.  To do this, you need to calculate annual rent (12*current market monthly rent) and then determine value with a multiplier.  What multiplier is implied by the base pro forma.  Change the market rent assumption so that it increases at a faster rate than inflation.  What does this do to the IRR?  
